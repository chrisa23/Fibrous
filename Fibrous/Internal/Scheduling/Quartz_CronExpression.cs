...
namespace Quartz
{
    ...
    [Serializable]
    internal class CronExpression : IDeserializationCallback, ISerializable
    {
        /// <summary>
        ...
        {
            if (cronExpression == null) throw new ArgumentException("cronExpression cannot be null");
        ...
        protected CronExpression(SerializationInfo info, StreamingContext context)
        {
            ...
            switch (version)
            {
                ...
                case 1:
                    ...
                    if (!string.IsNullOrEmpty(timeZoneId)) timeZone = TimeZoneUtil.FindTimeZoneById(timeZoneId);
                    break;
                default:
        ...
        {
            var withoutMilliseconds = new DateTimeOffset(dateUtc.Year, dateUtc.Month, dateUtc.Day, dateUtc.Hour,
                dateUtc.Minute, dateUtc.Second, dateUtc.Offset);
        ...
        public virtual DateTimeOffset? GetNextInvalidTimeAfter(DateTimeOffset date)
        {
            ...
            while (difference == 1000)
            {
                ...
                if (difference == 1000) lastDate = newDate.Value;
            }
        ...
        protected void BuildExpression(string expression)
        {
            ...
            try
            {
                ...
                var exprsTok = expression.Split(splitSeparators, StringSplitOptions.RemoveEmptyEntries);
                foreach (var exprTok in exprsTok)
                {
                    ...
                    var vTok = expr.Split(commaSeparator);
                    foreach (var v in vTok) StoreExpressionVals(0, v, exprOn);

                    exprOn++;
                }

                if (exprOn <= DayOfWeek) throw new FormatException("Unexpected end of expression.");
        ...
        protected virtual int StoreExpressionVals(int pos, string s, int type)
        {
            ...
            if (c >= 'A' && c <= 'Z' && !s.Equals("L") && !s.Equals("LW") && !regex.IsMatch(s))
            {
                ...
                if (type == Month)
                {
                    ...
                    if (sval <= 0) throw new FormatException($"Invalid Month value: '{sub}'");
                    if (s.Length > i + 3)
                    {
                        c = s[i + 3];
                        if (c == '-')
                ...
                else if (type == DayOfWeek)
                {
                    ...
                    if (sval < 0) throw new FormatException($"Invalid Day-of-Week value: '{sub}'");
                    if (s.Length > i + 3)
                    {
                        c = s[i + 3];
                        if (c == '-')
                        ...
                        else if (c == '/')
                        {
                            try
                            {
                                ...
                                if (everyNthWeek < 1 || everyNthWeek > 5) throw new Exception();
                            }
                            catch (Exception)
                            {
                                throw new FormatException(
            ...
            if (startsWithAsterisk || c == '/')
            {
                ...
                if (startsWithAsterisk) i++;
                c = s[i];
                if (c == '/')
                {
                    // is an increment specified?
                    i++;
                    if (i >= s.Length) throw new FormatException("Unexpected end of string.");
            ...
            if (c == 'L')
            {
                ...
                if (type == DayOfMonth && s.Length > i)
                {
                    ...
                    if (c == '-')
                    {
                        ...
                        if (lastdayOffset > 30) throw new FormatException("Offset from last day must be <= 30");
                        i = vs.pos;
                    }
        ...
        {
            if (incr > 59 && (type == Second || type == Minute)) throw new FormatException($"Increment > 60 : {incr}");
            if (incr > 23 && type == Hour) throw new FormatException($"Increment > 24 : {incr}");
            if (incr > 31 && type == DayOfMonth) throw new FormatException($"Increment > 31 : {incr}");
            if (incr > 7 && type == DayOfWeek) throw new FormatException($"Increment > 7 : {incr}");
            if (incr > 12 && type == Month) throw new FormatException($"Increment > 12 : {incr}");
        }
        ...
        protected virtual int CheckNext(int pos, string s, int val, int type)
        {
            ...
            if (c == 'L')
            {
                if (type == DayOfWeek)
                {
                    if (val < 1 || val > 7) throw new FormatException("Day-of-Week values must be between 1 and 7");
                    lastdayOfWeek = true;
                }
                else
                {
        ...
        {
            if (monthMap.ContainsKey(s)) return monthMap[s];
        ...
        {
            if (dayMap.ContainsKey(s)) return dayMap[s];
            ...
            while (!gotOne)
            {
                ...
                else if (dayOfWSpec && !dayOfMSpec)
                {
                    ...
                    if (lastdayOfWeek)
                    {
                        ...
                        while (day + daysToAdd + 7 <= lDay) daysToAdd += 7;
                    ...
                    else if (everyNthWeek != 0)
                    {
                        ...
                        if (st.Count > 0) dow = st.First();

                        var daysToAdd = 0;
                        if (cDow < dow) daysToAdd = dow - cDow + 7 * (everyNthWeek - 1);
                        if (cDow > dow) daysToAdd = dow + (7 - cDow) + 7 * (everyNthWeek - 1);
...
//                t = -1;
        ...
        {
            return new DateTimeOffset(time.Year, time.Month, time.Day, time.Hour, time.Minute, time.Second,
                time.Offset);
        }
        ...
        protected static DateTimeOffset SetCalendarHour(DateTimeOffset date, int hour)
        {
            ...
            var d = new DateTimeOffset(date.Year, date.Month, date.Day, hourToSet, date.Minute, date.Second,
