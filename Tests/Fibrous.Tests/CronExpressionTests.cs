...
public class CronExpressionTest
{
    ...
    {
        CronExpression cronExpression = new("0 15 10 * * ? 2005");
    ...
    {
        CronExpression cronExpression = new("0 15 10 L-2 * ? 2010");
    ...
    {
        CronExpression cronExpression = new("0 15 23 * * ?");
    ...
    public void TestCronExpressionPassingYear()
    {
        ...
        CronExpression ce = new("0 55 15 1 * ?");
    ...
    {
        CronExpression cronExpression = new("0 0 12 ? * MON-FRI");
        int[] arrJuneDaysThatShouldFire =
        {
            1, 4, 5, 6, 7, 8, 11, 12, 13, 14, 15, 18, 19, 20, 22, 21, 25, 26, 27, 28, 29
        };
        List<int> juneDays = new(arrJuneDaysThatShouldFire);
    ...
    {
        CronExpression cronExpression = new("0 0 12 ? * FRI");
    ...
    {
        CronExpression cronExpression = new("0 0 12 ? * FRI/2");
        DateTimeOffset? nextRunTime = cronExpression.GetTimeAfter(DateTimeOffset.Now);
        DateTimeOffset? nextRunTime2 = cronExpression.GetTimeAfter((DateTimeOffset)nextRunTime);
    ...
    {
        CronExpression cronExpression = new("0 0 12 ? * THU,FRI/2");
        DateTimeOffset? nextRunTime = cronExpression.GetTimeAfter(DateTimeOffset.Now);
        DateTimeOffset? nextRunTime2 = cronExpression.GetTimeAfter((DateTimeOffset)nextRunTime);
    ...
    {
        CronExpression cronExpression = new("0 0 12 L * ?");
    ...
    public void TestHourShift()
    {
        ...
        CronExpression cronExpression = new("0/5 * * * * ?");
        DateTimeOffset cal = new(2005, 6, 1, 1, 59, 55, TimeSpan.Zero);
        DateTimeOffset nextExpectedFireTime = new(2005, 6, 1, 2, 0, 0, TimeSpan.Zero);
        Assert.AreEqual(nextExpectedFireTime, cronExpression.GetTimeAfter(cal).Value);
    }
    ...
    {
        // QRTZNET-28
        CronExpression cronExpression = new("* * 1 * * ?");
    ...
    {
        // QRTZNET-85
        CronExpression cronExpression = new("0 12 4 ? * 3");
    ...
    {
        CronExpression ce = new("0 30 10-13 ? * FRI#3");
        DateTime start = new(2008, 12, 19, 0, 0, 0);
        for (int i = 0; i < 200; ++i)
        {
    ...
    private static void AssertParsesForField(string expression, int constant)
    {
        try
        {
            SimpleCronExpression cronExpression = new(expression);
    ...
    {
        CronExpression expression = new("0 0 0 29 * ?");
    ...
    {
        CronExpression expression = new("0 5 13 5W 1-12 ?");
        DateTimeOffset test = new(2009, 3, 8, 0, 0, 0, TimeSpan.Zero);
    ...
    {
        CronExpression expression = new("0 15 * * * ?");
    ...
    public static DaylightTime GetDaylightChanges(TimeZoneInfo InTimeZoneInfo, int InYear)
    {
        ...
        DaylightTime outDaylightTime = new(
    ...
    public void TestDaylightSavingsDoesNotMatchAnHourBefore()
    {
        ...
        CronExpression expression = new("0 15 15 5 11 ?");
        expression.TimeZone = est;

        DateTimeOffset startTime = new(2012, 11, 4, 0, 0, 0, TimeSpan.Zero);

        DateTimeOffset? actualTime = expression.GetTimeAfter(startTime);
        DateTimeOffset expected = new(2012, 11, 5, 15, 15, 0, TimeSpan.FromHours(-5));
    ...
    public void TestDaylightSavingsDoesNotMatchAnHourBefore2()
    {
        //another case
        TimeZoneInfo est = TimeZoneUtil.FindTimeZoneById("Eastern Standard Time");
        CronExpression expression = new("0 0 0 ? * THU");
        expression.TimeZone = est;

        DateTimeOffset startTime = new(2012, 11, 4, 0, 0, 0, TimeSpan.Zero);

        DateTimeOffset? actualTime = expression.GetTimeAfter(startTime);
        DateTimeOffset expected = new(2012, 11, 8, 0, 0, 0, TimeSpan.FromHours(-5));
