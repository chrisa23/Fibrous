...
namespace Fibrous.Collections
{
        ...
        IRequestPort<Func<TKey, bool>, KeyValuePair<TKey, T>[]>, IDisposable
    {
        ...
        public Task<KeyValuePair<TKey, T>[]> SendRequestAsync(Func<TKey, bool> request) => _request.SendRequestAsync(request);

        public Task<Result<KeyValuePair<TKey, T>[]>> SendRequestAsync(Func<TKey, bool> request, TimeSpan timeout) =>
        ...
        public void AddRange(IEnumerable<KeyValuePair<TKey, T>> items) =>
            _fiber.Enqueue(() =>
            {
                List<KeyValuePair<TKey, T>> added = new List<KeyValuePair<TKey, T>>();
                List<KeyValuePair<TKey, T>> updated = new List<KeyValuePair<TKey, T>>();
