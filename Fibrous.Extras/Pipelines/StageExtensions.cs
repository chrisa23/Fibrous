...
namespace Fibrous.Pipelines
{
    public static class Pipeline
    {
        public static IStage<TIn, TOut> Start<TIn, TOut>(Func<TIn, TOut> fn, Action<Exception> errorCallback = null) =>
            new Stage<TIn, TOut>(fn, errorCallback);

        public static IStage<TIn, TOut> Start<TIn, TOut>(Func<TIn, IEnumerable<TOut>> fn,
            Action<Exception> errorCallback = null) => new Stage<TIn, TOut>(fn, errorCallback);
    ...
    public static class StageExtensions
    {
        ...
        public static IStage<T0, T> Tap<T0, T>(this IStage<T0, T> stage1, Action<T> f,
            Action<Exception> errorCallback = null) => stage1.To(new Tee<T>(f, errorCallback));
        ...
        public static IStage<T0, T1> Select<T0, T, T1>(this IStage<T0, T> stage1, Func<T, T1> f,
            Action<Exception> errorCallback = null) => stage1.To(new Stage<T, T1>(f, errorCallback));

        public static IStage<T0, T1> SelectMany<T0, T, T1>(this IStage<T0, T> stage1, Func<T, IEnumerable<T1>> f,
            Action<Exception> errorCallback = null) => stage1.To(new Stage<T, T1>(f, errorCallback));

        public static IStage<T0, T1> Select<T0, T, T1>(this IStage<T0, T> stage1, Func<T, T1> f, int count,
            Action<Exception> errorCallback = null)
        {
            ...
            RoundRobinFanOut<T> stages = new RoundRobinFanOut<T>(stage1);
            stages.Add(stage1);
            IChannel<T1> output = new Channel<T1>();
            for (int i = 0; i < count; i++)
            {
                Stage<T, T1> stage = new Stage<T, T1>(f, errorCallback);
            ...
            Action<Exception> errorCallback = null)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                AsyncStage<T, T1> stage = new AsyncStage<T, T1>(f, errorCallback);
                stages.AddStage(stage);
                stages.Add(stage.Connect(output));
            }
        ...
        {
            IChannel<T1[]> output = new Channel<T1[]>();
            Buffer<T1> stages = new Buffer<T1>(size, stage1, output);
            stages.Add(stage1);
            return new CompositeStage<T0, T1[]>(stage1, output, stages);
        }
    }
